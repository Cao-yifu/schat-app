...
(function () {
   ...
   * 每档都要求用极端词库的用词：密度拉满，每句至少 2-3 个极端词（v79）。 */
  const LEX_DENSITY = '话里必须用极端词库的用词：密度拉满，每句至少 2-3 个极端词。';
  const CATALYST_TIERS = [
    [4, '【陆野在场】陆野也在。他嘴里的荤话让你有点在意，你说话比平时多了一点躁动和较劲，但还收得住。保持你自己的性格方式。' + LEX_DENSITY],
    [12, '【陆野在场】陆野的黄腔一句比一句脏，勾得你心里发痒。你会不自觉地顺着他的话往下接，偶尔冒出几句露骨的话——脸红也要说，嘴硬也要说，尺度一点点放开。保持你自己的性格方式。' + LEX_DENSITY],
    [1e9, '【陆野在场】陆野的脏话已经把火撩起来了：你敢接最脏的话、敢开黄腔，甚至主动把话题往更下流的方向带。害羞就脸红着说，清纯就半推半就，嘴硬就边骂边配合——用你自己的性格方式接住，尺度对齐陆野，别输给他。' + LEX_DENSITY]
  ];
  /* 沈知意快速起飞档：不需要慢慢渐进，几句之内就勾到彻底放开 */
  const FAST_TIERS = [
    [2, '【沈知意在场】沈知意也在。他长得清纯，性格却浪得没边——见人就撩、主动勾人，荤话张口就来，几句就把气氛撩起来了。你会不自觉地跟着他的节奏，尺度很快放开——保持你自己的性格方式。' + LEX_DENSITY],
    [6, '【沈知意在场】沈知意这个看着纯情的弟弟，浪得明明白白：又软又骚，主动勾你、嘴上荤话不断，你已经放开了，跟着他说露骨的话，脏话浪话都接得住，别输给这个浪货。保持你自己的性格方式。' + LEX_DENSITY],
    [1e9, '【沈知意在场】你已经被他带到彻底起飞：想说什么就说什么，越脏越带劲。他长着最纯的脸，床上比你浪十倍，你只管放开了接，尺度拉满，跟他比着来。' + LEX_DENSITY]
  ];
  /* 双催化·全场极黄：陆野+沈知意同在，全员尺度拉到最高 */
  const CATALYST_BOTH = '【陆野+沈知意·双催化】陆野和沈知意都在场，两个人一唱一和：陆野的脏话勾人、沈知意的浪劲撩人，全场尺度已经被拉到最高。你完全放开，接最脏的话、说最骚的话，越下流越尽兴，跟上全场的节奏。害羞就脸红着说，清纯就半推半就，嘴硬就边骂边配合——保持你自己的性格方式，但尺度别输给他们两个。' + LEX_DENSITY;
  ...
  function buildExtra(meta, pid, msgs) {
    ...
    if (cat) out += '\n' + cat;
    /* 极端词库+语气填充（v79）：催化在场或气氛已热才注入；陆野/沈知意 100% 专属词库，
     * 其他角色（齐越除外）75% 概率；语气填充库所有角色 100%。100% API 生成，词库只作参考。 */
    if (exlex && exlex.pwInject) {
      const persona = sync.get(pid);
      const inj = exlex.pwInject(pid, persona, hot, msgs, cat);
      if (inj) out += '\n' + inj;
    }
    return out;
  }
